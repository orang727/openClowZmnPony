...
**必须通过 `mcporter call` 命令调用 Pencil 工具。禁止直接调用 mcp-server 二进制文件或通过管道 (pipe/echo) 发送 JSON-RPC。**

错误示例（禁止）：
```
echo '{"jsonrpc":"2.0",...}' | /Applications/Pencil.app/.../mcp-server-darwin-arm64 --app desktop
cat file.json | /Applications/Pencil.app/.../mcp-server-darwin-arm64 --app desktop
```

## 调用方式

所有 Pencil 工具**只能**通过 `mcporter call pencil.<工具名>` 调用：
...
```bash
mcporter call pencil.open_document filePathOrTemplate:new
mcporter call pencil.open_document filePathOrTemplate:/path/to/file.pen
```
